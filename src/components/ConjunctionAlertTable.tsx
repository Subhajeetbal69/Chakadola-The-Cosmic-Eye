...
import { exportConjunctionsToCSV } from '../utils/csvExport';
...
const ConjunctionRow = React.memo<{
  ...
  onExportSingle?: (e: React.MouseEvent, conj: ConjunctionEvent) => void;
}>(({
  conjunction: c,
  isSelected,
  isExpanded,
            ...
            <div className="bg-slate-900 border border-cyan-500/40 rounded-2xl p-4 sm:p-5 space-y-4 text-xs font-mono text-slate-200 shadow-2xl">
              ...
              <div className="pt-2 flex flex-wrap items-center justify-between gap-3 border-t border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={(e) => onExportSingle?.(e, c)}
                    className="px-3.5 py-1.5 rounded-xl bg-slate-850 hover:bg-slate-750 text-slate-300 hover:text-white border border-slate-700/80 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm active:scale-95 hover:border-emerald-500/50"
                    title="Export this single threat encounter telemetry as CSV"
                  >
...
export const ConjunctionAlertTable: React.FC<ConjunctionAlertTableProps> = React.memo(({
  ...
  const handleExportCSV = useCallback(() => {
    if (processedConjunctions.length === 0) return;
    const filterSuffix = filterLevel !== 'ALL' ? `_${filterLevel.toLowerCase()}` : '';
    const dateStr = new Date().toISOString().slice(0, 10);
    const filename = `conjunction_risk_assessment${filterSuffix}_${dateStr}.csv`;
    const res = exportConjunctionsToCSV(processedConjunctions, filename);
    if (res.success) {
      setExportedSuccess(true);
      setExportedCount(res.count);
      setTimeout(() => setExportedSuccess(false), 2500);
      onExportSuccess?.(res.count, res.filename);
    }
  }, [processedConjunctions, filterLevel, onExportSuccess]);

  const handleExportAllCSV = useCallback(() => {
    if (safeConjunctions.length === 0) return;
    const dateStr = new Date().toISOString().slice(0, 10);
    const filename = `conjunction_risk_assessment_all_${dateStr}.csv`;
    const res = exportConjunctionsToCSV(safeConjunctions, filename);
    if (res.success) {
      setExportedSuccess(true);
      setExportedCount(res.count);
      setTimeout(() => setExportedSuccess(false), 2500);
  ...
  }, [safeConjunctions, onExportSuccess]);

  const handleExportSingle = useCallback((e: React.MouseEvent, conj: ConjunctionEvent) => {
    e.stopPropagation();
    const nameA = conj.objectA?.name?.replace(/[^a-zA-Z0-9_-]/g, '_') || 'primary';
    const nameB = conj.objectB?.name?.replace(/[^a-zA-Z0-9_-]/g, '_') || 'secondary';
    const filename = `conjunction_threat_${nameA}_vs_${nameB}.csv`;
    const res = exportConjunctionsToCSV([conj], filename);
    if (res.success) {
      onExportSuccess?.(1, res.filename);
    }
  }, [onExportSuccess]);
  ...
  return (
    ...
    >
      ...
      <div className="p-4 sm:p-5 border-b border-slate-800 flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-[#0f172a]">
        ...
        {/* Filter Tabs, Search, Export & Sort */}
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-slate-900 text-xs text-slate-200 pl-8 pr-3 py-1.5 rounded-xl border border-slate-700 focus:outline-none focus:border-blue-500 transition-colors w-48 sm:w-52"
            />
          </div>
          ...
          {/* Export CSV Action Group */}
          <div className="flex items-center gap-1.5 pl-1">
            <button
              id="btn-export-conjunctions-csv"
              onClick={handleExportCSV}
              disabled={processedConjunctions.length === 0}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all shadow-md active:scale-95 ${
                exportedSuccess
                  ? 'bg-emerald-600 text-white shadow-emerald-500/30 font-bold'
                  : 'bg-emerald-600 hover:bg-emerald-500 text-white border border-emerald-400/40 hover:shadow-lg hover:shadow-emerald-500/20'
              } disabled:opacity-40 disabled:pointer-events-none`}
              title={`Export ${processedConjunctions.length} conjunction alerts as CSV file for offline trajectory & risk analysis`}
            >
              {exportedSuccess ? (
                <>
                ...
                <>
                  ...
                  <span className="bg-emerald-800/80 text-emerald-100 text-[10px] px-1.5 py-0.5 rounded font-mono font-bold">
                    {processedConjunctions.length}
                  </span>
                </>
              )}
            </button>

            {filterLevel !== 'ALL' && safeConjunctions.length !== processedConjunctions.length && (
              <button
                onClick={handleExportAllCSV}
                className="px-2.5 py-1.5 rounded-xl text-[11px] font-medium text-slate-300 hover:text-white bg-slate-900 hover:bg-slate-800 border border-slate-700/80 transition-colors flex items-center gap-1"
                title={`Export ALL (${safeConjunctions.length}) conjunction alerts without current filter`}
              >
                <FileSpreadsheet className="w-3 h-3 text-emerald-400" />
                <span>All ({safeConjunctions.length})</span>
              </button>
            )}
          </div>
        </div>
