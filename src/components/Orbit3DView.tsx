...
export const Orbit3DView: React.FC<Orbit3DViewProps> = React.memo(({
  ...
  useEffect(() => {
    if (selectedObject && selectedObject.id !== caughtObjectRef.current?.id) {
  ...
  const catchObject = useCallback(
    (obj: TrackedObjectSummary, shouldFollow: boolean = false) => {
      setCaughtObject(obj);
      if (shouldFollow) setIsFollowCam(true);
      ...
      if (currentPos.length() > 0.1) {
        cameraSphericalRef.current.target.copy(shouldFollow ? currentPos : new THREE.Vector3(0, 0, 0));
        cameraSphericalRef.current.theta = Math.atan2(currentPos.x, currentPos.z);
        cameraSphericalRef.current.phi = Math.acos(
          Math.max(-0.95, Math.min(0.95, currentPos.y / currentPos.length()))
        );
        targetRadiusRef.current = Math.max(14, Math.min(26, cameraSphericalRef.current.radius));
        updateCameraPosition();
      }
    },
    [getObjectInterpolatedPosition, onSelectObject, updateCameraPosition]
  );
  ...
  const inspectedObj = caughtObject || hoveredObject || (objects.length > 0 ? objects[0] : null);
  const currentSpeedKmS = inspectedObj?.speedKmS ?? 7.68;
          ...
          <div className="flex items-center gap-0.5 bg-slate-950/90 backdrop-blur-md p-1 border border-white/10 rounded-xl text-[10px] shadow-xl">
            ...
            {caughtObject && (
              <button
                ...
                title="Lock camera to follow and track caught satellite"
